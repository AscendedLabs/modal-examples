...
<body>
    ...
    <div class="main-content">
      ...
      <div class="inspector-panel">
        <div class="inspector-tabs">
          ...
          <div class="inspector-tab" data-tab="vocal" onclick="switchTab('vocal')">Vocal</div>
            ...
            <div class="inspector-section">
              ...
              <div class="param-group">
                ...
                <select id="genre">
                  ...
                  <option>R&B</option><option>Country</option><option>Blues</option>
                </select>
              </div>
              <div class="param-group">
                <label class="param-label">Mood</label>
                <select id="mood">
                  <option value="">Auto</option>
                  <option>Happy</option><option>Sad</option><option>Energetic</option>
                  <option>Calm</option><option>Epic</option><option>Romantic</option>
                </select>
              </div>
              <div class="param-group">
              ...
              <div class="param-group">
                <label class="param-label">
                  <span>Steps</span>
                  <span class="param-value"><span id="stepsValue">60</span></span>
                </label>
                <input type="range" id="steps" min="27" max="100" value="60" oninput="updateValue('steps')" />
              </div>
              <div class="btn-group">
                <button class="btn btn-primary" style="flex:1" id="generateBtn" onclick="generateMusic()">🎵 Generate</button>
              </div>
              <div class="btn-group">
                <button class="btn btn-secondary" onclick="generateVariation()">🎲 Variation</button>
                <button class="btn btn-secondary" onclick="generateExtend()">➕ Extend +10s</button>
              </div>
          ...
          <div id="tab-effects" class="tab-content" style="display:none;">
            ...
            <div class="inspector-section" id="eqSection">
              ...
              <div class="param-group">
                ...
                <input type="range" id="eqLow" min="-12" max="12" value="0" step="0.5" oninput="updateValue('eqLow')" />
              </div>
              <div class="param-group">
                <label class="param-label">
                  <span>Mid</span>
              ...
              <div class="param-group">
                ...
                <input type="range" id="eqHigh" min="-12" max="12" value="0" step="0.5" oninput="updateValue('eqHigh')" />
              </div>
            </div>
          </div>
          ...
          <div id="tab-vocal" class="tab-content" style="display:none;">
            <div class="inspector-section">
              ...
              <div class="param-group">
                ...
                <input type="range" id="pitchCorrection" min="0" max="100" value="50" oninput="updateValue('pitchCorrection')" />
              </div>
              <div class="param-group">
                <label class="param-label">
                  <span>Vibrato</span>
                ...
                <input type="range" id="vibrato" min="0" max="100" value="30" oninput="updateValue('vibrato')" />
              </div>
              <div class="param-group">
                <label class="param-label">Lyrics</label>
                <textarea id="lyrics" placeholder="[verse] Walking down the road...&#10;[chorus] Feel the rhythm"></textarea>
              </div>
            </div>
          </div>
  ...
  <script>
    ...
    async function generateVariation() {
      await generateMusic('variation', 0);
    }
    ...
    function renderTracks() {
      ...
      list.innerHTML = tracks.map(track => `
        <div class="track-item ${selectedTrack === track.id ? 'selected' : ''}" onclick="selectTrack('${track.id}')">
          ...
          <div class="track-controls">
            <button class="track-btn ${track.muted ? 'active' : ''}" onclick="toggleMute('${track.id}', event)">M</button>
            <button class="track-btn ${track.solo ? 'active' : ''}" onclick="toggleSolo('${track.id}', event)">S</button>
          </div>
        </div>
      `).join('');
      ...
      timeline.innerHTML = tracks.map(track => `
        <div class="timeline-track">
          ...
          ${track.clips.map((clip, idx) => `
            <div class="timeline-clip" style="left: ${clip.position}%; width: ${clip.width}%;">
    ...
    function renderMixer() {
      ...
      mixer.innerHTML = tracks.map((track, idx) => `
        <div class="mixer-channel">
          <div class="channel-name">${track.name}</div>
          <div class="channel-fader" data-track="${track.id}">
            <div class="fader-thumb" style="bottom: ${track.volume * 100}%"></div>
          </div>
          <div class="channel-value">${Math.round((track.volume * 2 - 1) * 12)} dB</div>
        </div>
      `).join('') + `
    ...
    function initPianoRoll() {
      ...
      for (let oct = 5; oct >= 3; oct--) {
        notes.forEach(note => {
          ...
          div.className = 'piano-key' + (blackNotes.includes(note) ? ' black' : '');
    ...
    function initEffectsRack() {
      ...
      rack.innerHTML = `
        <div class="effect-slot">
          ...
          <div class="effect-toggle ${effects.eq.enabled ? 'on' : ''}" onclick="toggleEffect('eq')"></div>
        </div>
        <div class="effect-slot">
          <div class="effect-name">🗜️ Compressor</div>
          <div class="effect-toggle ${effects.compressor.enabled ? 'on' : ''}" onclick="toggleEffect('compressor')"></div>
        </div>
        <div class="effect-slot">
          <div class="effect-name">🌊 Reverb</div>
          <div class="effect-toggle ${effects.reverb.enabled ? 'on' : ''}" onclick="toggleEffect('reverb')"></div>
        </div>
      `;
    }
